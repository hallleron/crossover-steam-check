...
function scheduleSave() {
  if (saveTimer || saveDisabled) return;
...
function flushCache() {
  ...
  try {
    ...
    if (
      ...
      saveDisabled = true;
      console.warn(
        `[crossover] ${CACHE_DIR} is not writable — disk persistence ` +
          'disabled for this session, in-memory cache still works. ' +
          'Mount a writable volume or run the container as a user that ' +
          'owns the mount target to re-enable persistence.',
      );
      return;
    }
...
// Defense in depth: only accept http(s) URLs on the codeweavers.com host
// when treating a scraped href as the canonical "source" URL we'll later
// hand back to the frontend as an <a href>. If the page ever served a
// surprise (javascript:, attacker.com, etc.), this drops it.
function safeCodeweaversUrl(raw) {
  try {
    const u = new URL(raw, CW_BASE);
    if (u.protocol !== 'https:' && u.protocol !== 'http:') return null;
    if (u.hostname !== 'www.codeweavers.com') return null;
    return u.toString();
  } catch {
    return null;
  }
...
function parseSearchResults(html) {
  ...
  $('a[href*="/compatibility/crossover/"]').each((_, el) => {
    ...
    const safeUrl = safeCodeweaversUrl(href);
    if (!safeUrl) return;
